...
export function contentRequest(cwd) {
    try {
        return request('api/contents/' + cwd, {
            method: 'GET'
        }).then(rvalue => {
            //console.log(rvalue);
            return rvalue;
...
export function projectInfoRequest(cwd) {
    const dataToSend = { CWD: cwd };
    try {
        return request('swan/project/info', {
        ...
        console.error(`Error on POST 'swan/project/info'+ ${dataToSend}.\n${reason}`);
    }
}
/**
